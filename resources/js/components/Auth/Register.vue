...
<!--      <p class="text-yellow-400 text-xs text-center italic py-2" v-if="errorMessage">{{errorMessage}}</p>-->
       <div>
         <input @input="validateSingly($event, 'first_name')" :value="data.first_name" class="w-full block bg-transparent rounded-lg border border-4 text-gray-300 px-4 py-3 mb-3 focus:outline-none placeholder-gray-500"  type="text" placeholder="First Name">
         <div v-if="errors" class="pb-2">
            <p v-for="error in errors['first_name']" :key="error" class="text-xs text-yellow-500 italic">{{error}}</p>
          </div>
       </div>
       <div>
         <input @input="validateSingly($event, 'last_name')" :value="data.last_name" class="w-full block bg-transparent rounded-lg border border-4 text-gray-300 px-4 py-3 mb-3 focus:outline-none placeholder-gray-500"  type="text" placeholder="Last Name">
         <div v-if="errors" class="pb-2">
            <p v-for="error in errors['last_name']" :key="error" class="text-xs text-yellow-500 italic">{{error}}</p>
          </div>
       </div>
       <div>
       ...
       <div>
        ...
        <div class="w-full">
          <SubmitButton title="Signup" processBtnClass="bg-yellow-100 py-2 text-gray-700 text-xl rounded-lg" btnClass="bg-gray-200 rounded-lg text-gray-900 font-bungee text-xl hover:bg-yellow-200 hover:text-gray-800 rounded-bl-xs rounded-br-xs font-bold py-2 focus:outline-none focus:shadow-outline" :processing="processing" @send="submit"/>
        </div>
    </div>
</template>
  ...
  methods: {
    validateSingly(event, attribute) {
     ...
     async submit() {
      ...
      try {
        const response = await this.post(this.http.auth.register, this.data);
